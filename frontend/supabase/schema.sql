...
create or replace function public.get_recent_submissions(limit_count int default 10)
returns table(
  id uuid,
  ticket_ref text,
  submitter_name text,
...
to authenticated
with check (bucket_id = 'site-assets' and public.is_ymca_admin());

drop policy if exists "Admins can update site assets" on storage.objects;
create policy "Admins can update site assets"
on storage.objects for update
to authenticated
using (bucket_id = 'site-assets' and public.is_ymca_admin())
with check (bucket_id = 'site-assets' and public.is_ymca_admin());
...
-- Ask PostgREST/Supabase API to refresh its schema cache after RPC changes.
